...
ImageEditor.processImage = function(args){
	...
	request.onreadystatechange = function(){
		if (request.readyState == 4){
			...
			if (json.FOUND) {
				...
				if (ImageEditor.w > 700 || ImageEditor.h > 600) {
					var imagediv = document.getElementById("imagewrap");
					
					imagediv.style.overflow="hidden";
					imagediv.style.width = "650px";
	...
	request.send("id=" + ImageEditor.imageId + ((args) ? "&" + args : ""));
};
ImageEditor.loadImage = function(){
	ImageEditor.loaderImage.setAttribute("src", "../viewImage/?id=" + ImageEditor.imageId + "&t=" + (new Date).getTime());
};
...
ImageEditor.resize = function(){
	...
	ImageEditor.processImage("imgaction=resize&w=" + w + "&h=" + h);
};
ImageEditor.rotate = function(degrees){
	ImageEditor.processImage("imgaction=rotate&degrees=" + degrees);
};
ImageEditor.viewActive = function(){
	ImageEditor.processImage("action=viewActive");
};
ImageEditor.revertToOriginal = function(){
	ImageEditor.processImage("imgaction=revert");
};
/*
...
ImageEditor.crop = function(){
	...
	ImageEditor.processImage("imgaction=crop&x=" + x + "&y=" + y + "&w=" + w + "&h=" + h);
};
...
ImageEditor.init = function(id){
	if (!id) return;
    ImageEditor.imageId = id || "";
