/****
*		Hired Jackass: Matthew Gaddis <matthew@spiremedia.com>
*		Created: 2006-02-01
*		Updated: 2006-07-21
		...
		for ( i = 0; i < this.CONTAINMENT_CLASSNAMES.length; i++ )
		{
			...
			for ( j = 0; j < containmentElements.length; j++ )
			{
				...
				for( k = 0; k < buttonElements.length; k++ )
				{
					...
					{
						// when using ajax to reload widgets, check that they are not already being observed for 
						//this event and only add the even if the button is not allready in the prototype events list
						//dre 20071022
						var found = false;
						for (var zz = 0; zz < Event.observers.length; zz++){
							if (Event.observers[zz][0] == buttonElement && Event.observers[zz][1] == 'click'){
								found = 1;
								break;								
							}
						}
						if (! found) Event.observe(buttonElement, 'click', this.options.onclick.bindAsEventListener(this));
					}
				}
			}
