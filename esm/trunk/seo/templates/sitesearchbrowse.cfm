<cfsilent>
	...
	<cfif isdataItemSet('month')>
		<cfset lcl.id = getDataItem('year') & '_' & getDataItem('month')>
	<cfelse>
		<cfset lcl.id = 0>
	</cfif>
	...
	<cfoutput query="lcl.list" group="year">
        ...
        <cfsavecontent variable="lcl.s.html">
			...
			<cfif lcl.list.recordcount>
				<cfoutput>
					<cfif lcl.id EQ lcl.list.year & '_' & lcl.list.month>
						<cfset lcl.acc.setSelected(lcl.count)>
					</cfif>
					<li><a <cfif lcl.id EQ lcl.list.year & '_' & lcl.list.month>class="selected"</cfif> href="../siteSearchviewByMonth/?month=#lcl.list.month#&year=#lcl.list.year#">#monthasstring(lcl.list.month)#</a></li>
				</cfoutput>
			<cfelse>
				&nbsp;No searches have occurred.
			</cfif>
			</ul>
			</div>
		</cfsavecontent>
		
		<cfset lcl.acc.add("Local Search : " & lcl.list.year,lcl.s.html)>
	</cfoutput>
    
	<cfif lcl.list.recordcount EQ 0>
		<cfset lcl.acc.add("No Searches Available","")>
	</cfif>
