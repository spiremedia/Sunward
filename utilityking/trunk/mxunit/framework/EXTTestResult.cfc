<cfcomponent extends="TestResult">
      ...
      function toggleExpandCollapse(btn, pressed){
        ...
        view: view,
            ...
            columns: [
                ...
                {header: "Method", width: 100, sortable: true, dataIndex: 'method'},
  ...
  <cffunction name="resultsToJS" access="private" output="false">
    ...
    <cfloop from="1" to="#ArrayLen(results)#" index="test">
      <cfif NOT problemsOnly OR (problemsOnly AND isProblem(results[test]))>
        <cfset thisJSON = "[#test#,'#results[test].Component#','#addMethodNameJS(results[test])#','#results[test].TestStatus#',#results[test].Time#,'#addErrorJS(results[test].Error)#','#addOutputLink(test,results[test])#']">
  ...
  <cffunction name="addMethodNameJS" output="false" access="private" returntype="string">
    ...
    <cfset link = "<a href='#cu.getContextRootPath()##fileURL#&method=runTestRemote&output=extjs'>#testStruct.testname#</a>">
